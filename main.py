...
def index():
    return '''
        <h1>Добро пожаловать на сайт о технологических зависимостях</h1>
        <p><a href="/facts">Узнать случайный факт</a></p>
        <p><a href="/secret">Тайная страница</a></p>
    '''

@app.route("/facts")
def facts():
    return f"<h2>Факт дня:</h2><p>{random.choice(facts_list)}</p><a href='/'>Назад</a>"

@app.route("/secret")
def secret():
    return "<h1>Ты нашёл тайную страницу! </h1><p>А ты знал, что ты молодец?</p><a href='/'>На главную</a>"
